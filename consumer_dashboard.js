...
const bookings = [
    {
        ...
        date: '2025-05-10', // manually set today's date for testing
        status: 'upcoming',
        checkInTime: null // Added to track check-in time
    },
    {
        id: 'B002',
        area: 'Area B',
        ...
        date: '2025-05-01',
        status: 'active',
        checkInTime: '2025-05-01T12:45:00' // Example check-in time
    },
    {
        id: 'B003',
        area: 'Area C',
        ...
        date: '2025-04-30',
        status: 'completed',
        checkInTime: '2025-04-30T02:15:00'
    }
];

// Function to check if the booking should be auto-cancelled or auto-checked out
function autoUpdateBookings() {
    const now = new Date();

    bookings.forEach(b => {
        // Auto Cancel Check-In after 30 minutes of booking time
        if (b.status === 'upcoming') {
            const bookingTime = new Date(`${b.date} ${b.time.split(' - ')[0]}`);
            const diffMinutes = (now - bookingTime) / 60000; // Difference in minutes
            if (diffMinutes > 30 && !b.checkInTime) {
                updateBookingStatus(b.id, 'cancelled');
            }
        }

        // Auto Check-Out after booking time ends
        if (b.status === 'active') {
            const endTime = new Date(`${b.date} ${b.time.split(' - ')[1]}`);
            const diffMinutes = (now - endTime) / 60000; // Difference in minutes
            if (diffMinutes > 0 && !b.checkInTime) {
...
// Call auto update every minute
setInterval(autoUpdateBookings, 60000); // Auto-update every minute
